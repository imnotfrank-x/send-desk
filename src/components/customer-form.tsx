  ...
  return (
    <form action={action} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <Field label="Código" name="code" defaultValue={customer?.code} error={state.fieldErrors?.code?.[0]} placeholder="CLI-004" />
        ...
        <label className="text-sm font-semibold text-slate-700 sm:col-span-2">Dirección<textarea name="address" defaultValue={customer?.address} rows={3} className={`${inputClass} h-auto py-3`} required />{state.fieldErrors?.address?.[0] && <span className="mt-1 block text-sm text-red-600">{state.fieldErrors.address[0]}</span>}</label>
...
function Field({ label, name, defaultValue, error, placeholder }: { label: string; name: string; defaultValue?: string; error?: string; placeholder?: string }) {
  return <label className="text-sm font-semibold text-slate-700">{label}<input name={name} defaultValue={defaultValue} placeholder={placeholder} className={inputClass} required />{error && <span className="mt-1 block text-sm text-red-600">{error}</span>}</label>;
}
