            ...
            return (
              <section key={index} className="min-h-[320px] border-b border-slate-300 p-5 odd:md:border-r">
                ...
                {!parcel ? <div className="grid h-56 place-items-center rounded-xl border border-dashed border-slate-200 text-sm text-slate-400">Posición disponible</div> : <div className="space-y-4 text-sm">
                  <div className="grid grid-cols-2 gap-3 rounded-lg bg-slate-50 p-3"><p><span className="block text-xs uppercase text-slate-500">Número</span><strong>{parcel.packageNumber}</strong></p><p><span className="block text-xs uppercase text-slate-500">Peso</span><strong>{parcel.weight.toFixed(2)} kg</strong></p><p className="col-span-2"><span className="block text-xs uppercase text-slate-500">Descripción</span>{parcel.description}</p></div>
